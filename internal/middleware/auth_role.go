...
func AuthRole(allowedRoles ...string) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if !ok {
			c.JSON(http.StatusForbidden, gin.H{"error": "Role not found in the token!"})
			c.Abort() // abort berguna agar handler selanjutnya di main.go tidak dieksekusi jika token tidak valid
			return
		}
		...
		for _, allowed := range allowedRoles {
			if role == allowed {
				...
				fmt.Println("User:", username, "dengan role:", roleList, "dari tenant:", tenantID, "mengakses endpoint ini.")

				c.Next() // lanjut eksekusi handler berikutnya, lihat tiap endpoint di routes
				return
			}
		}
